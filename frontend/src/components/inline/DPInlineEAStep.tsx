...
export default function DPInlineEAStep() {
  ...
  const hasTxSingleDe = transcriptomicsDS.length === 1 && transcriptomicsDS[0]
    ? !!(state.dpDeResults[transcriptomicsDS[0].id] || Object.entries(state.dpDeResults).find(([k]) => getBaseDatasetId(k) === getBaseDatasetId(transcriptomicsDS[0].id))?.[1])
    : false;
  const hasTxMetaResults = !!(
    state.dpDeResults["meta_transcriptomics"] ||
    state.dpDeResults[dpMetaKey] ||
    state.dpDeResults["meta"]
  );
  const canRunTxEA = transcriptomicsDS.length === 1
    ? hasTxSingleDe
    : (transcriptomicsDS.length > 1 && !state.dpDeMetaSkipped && (hasTxMetaResults || !!state.dpDeMetaMethod));

  const hasPrSingleDe = proteomicsDS.length === 1 && proteomicsDS[0]
    ? !!(state.dpDeResults[proteomicsDS[0].id] || Object.entries(state.dpDeResults).find(([k]) => getBaseDatasetId(k) === getBaseDatasetId(proteomicsDS[0].id))?.[1])
    : false;
  const hasPrMetaResults = !!(
    state.dpDeResults["meta_proteomics"] ||
    state.dpDeResults[dpMetaKey] ||
    state.dpDeResults["meta"]
  );
  const canRunPrEA = proteomicsDS.length === 1
    ? hasPrSingleDe
    : (proteomicsDS.length > 1 && !state.dpDeMetaSkipped && (hasPrMetaResults || !!state.dpDeMetaMethod));
  ...
  const cachedTx = state.dpInlineEnResultsCache["transcriptomics"];
  const cachedPr = state.dpInlineEnResultsCache["proteomics"];
  ...
  const [txDone, setTxDone] = useState(state.dpInlineEaDone || !!cachedTx);
  const [txSubmittedCfg, setTxSubmittedCfg] = useState<any>(
    cachedTx?.submittedCfg || ((state.dpInlineEaDone || !!cachedTx) ? {
  ...
  const [prDone, setPrDone] = useState(state.dpInlineEaDone || !!cachedPr);
  const [prSubmittedCfg, setPrSubmittedCfg] = useState<any>(
    cachedPr?.submittedCfg || ((state.dpInlineEaDone || !!cachedPr) ? {
      oraDatabase: enConfig.oraDatabase || [],
      gseaDatabase: enConfig.gseaDatabase || [],
      pValueCutoff: enConfig.pValueCutoff,
      qValueCutoff: enConfig.qValueCutoff,
  ...
  const isMetaActiveType = activeDsList.length > 1 && !state.dpDeMetaSkipped && !!state.dpDeMetaMethod;

  let activeRawDeData: any = [];
  if (isMetaActiveType) {
    const classMetaKey = `meta_${activeType}`;
    activeRawDeData = state.dpDeResults[classMetaKey] || state.dpDeResults["meta"] || state.dpDeResults[dpMetaKey] || [];
  } else {
    ...
    for (const ds of typeDatasets) {
      ...
      const entry = Object.entries(state.dpDeResults).find(([k]) => {
  ...
  const [subStep, setSubStep] = useState<"config" | "analysis">((state.dpInlineEaDone || !!cachedTx || !!cachedPr) ? "analysis" : "config");
  ...
  useEffect(() => {
    if (isOrganismLocked && lockedOrganism) {
      ...
      if (!enConfig.organism) {
        const initialOrg = state.dpAnnotationConfig?.selectOrganism || "Human (Homo sapiens)";
        dispatch({
          type: "EN_SET_CONFIG",
          patch: { organism: initialOrg }
        });
      }
    }
  }, [isOrganismLocked, lockedOrganism, enConfig.organism, state.dpAnnotationConfig?.selectOrganism, dispatch]);
  ...
  const isMetaActive = activeDsList.length > 1 && !state.dpDeMetaSkipped && !!state.dpDeMetaMethod;
  const isOnlyOraMeta = isMetaActive && (state.dpDeMetaMethod === "vote_counting" || state.dpDeMetaMethod === "shared_genes" || state.dpDeMetaMethod === "combine_pvalue");
  ...
  const handleRunAnalysis = async () => {
    ...
    try {
      ...
      const eaSource = (isMetaActive && activeDsList.length > 1) ? "inline_de_meta" : "inline_de";
      const firstDs = activeDsList[0];
      const geneIdCol = firstDs?.geneIdCol || "gene";
      const currentUserId = getSessionUserId();
      const datasetId = (isMetaActive && activeDsList.length > 1) ? `${currentUserId}_dp_meta` : (firstDs?.id || "");
      const results = await runInlineEAAPI(
        { ...enConfig, dataType: firstDs?.dataType, dataClass: activeType, parentModule: "dp", module: "ea", isInline: true } as any,
        eaSource,
        geneIdCol,
        datasetId,
        enConfig.geneIdType || firstDs?.geneIdType || "genename",
        direction,
        activeDsList.map(d => ({ id: d.id, dataType: d.dataType, name: d.name, parentModule: "dp", module: "ea", isInline: true }))
      );
      ...
      dispatch({ type: "SET_DP_INLINE_EA_DONE", done: true });
    } catch (err: any) {
  ...
  const isMetaSource = activeDsList.length > 1 && !state.dpDeMetaSkipped && !!state.dpDeMetaMethod;
  ...
  const currentRankBy = enConfig.rankByCol || (isMetaSource
    ? (state.dpDeMetaMethod === "effect_size" ? "Combined Effects Size" : "Combined LogFC")
    : "LogFC");
  ...
  useEffect(() => {
    ...
    if (!enConfig.rankByCol || 
      ...
      (isMetaSource && state.dpDeMetaMethod !== "effect_size" && enConfig.rankByCol === "Combined Effects Size") ||
      (isMetaSource && state.dpDeMetaMethod === "effect_size" && enConfig.rankByCol === "Combined LogFC")) {
      dispatch({
        type: "EN_SET_CONFIG",
        patch: { rankByCol: defaultRankBy }
      });
    }
  }, [isMetaSource, state.dpDeMetaMethod, enConfig.rankByCol, dispatch]);
      ...
      <>
        ...
        <div className="card" style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", minWidth: 180, maxWidth: 200, marginBottom: isGseaSelected ? 12 : 0 }}>
            ...
            {isOrganismLocked && (
              <span style={{ fontSize: 11, color: "var(--muted-foreground)", marginTop: 4, width: 300 }}>
                ...
                <thead>
                  {(!isMetaSource) ? (
                    ...
                    (() => {
                      ...
                      if (state.dpDeMetaMethod === "effect_size" || "Combined Effects Size" in firstRow || "hedges_g" in firstRow) {
                        return (
                          <tr>
                            <th>Feature</th>
                            <th>Fold Change</th>
                      ...
                      } else if (state.dpDeMetaMethod === "combine_pvalue" || "Combined LogFC" in firstRow || "combined" in firstRow) {
                        return (
                          <tr>
                            <th>Feature</th>
                            <th>Fold Change</th>
                      ...
                      } else if (state.dpDeMetaMethod === "vote_counting" || "votes" in firstRow) {
                        return (
                          <tr>
                            <th>Feature</th>
